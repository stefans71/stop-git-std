...
- **HEAD:** post-Phase 7 merge on `origin/main` (filled in by post-merge §Current state update). **Back-to-basics calibration rebuild COMPLETE.** Phases 0-7 all landed. Tree clean.
- **Session 12 work (2026-05-01):** Phase 6 + Phase 7 of back-to-basics-plan COMPLETE + MERGED + PUSHED. Phase 6: 5 cold-fork consumer tests on rendered MD; 5/5 match (pass bar ≥4); failure modes covered include over-cautious skill collections (entry 27 V13-3 missing_qualitative_context override survived) + under-cautious genuine criticals (Baileys/browser_terminal/freerouting all returned NO with calibrated reasoning). Phase 7: Simple Report HTML shipped — concept brief at `docs/simple-report-concept.md`, renderer at `docs/render-simple.py`, templates at `docs/templates-simple/{html.j2,md.j2,css}` (251-line CSS subset of design system), 22 new tests in `tests/test_render_simple.py`, 12 Simple Reports rendered to `docs/scans/catalog/GitHub-Scanner-<repo>-simple.{html,md}` (entries 16-27). Workflow integration commit applied 3 owner directives: CLAUDE.md Q3a flipped (V2.5-preview default; V2.4 legacy, no Simple Report); CLAUDE.md Q1 + post-scan options reframed (Simple Report is primary user-facing; long-form HTML auditor-only); Operator Guide §4 + §8 Phase 4 contract restated (4a render-md REQUIRED canonical / 4b render-simple REQUIRED user-facing / 4c render-html OPTIONAL auditor / 4d re-run record OPTIONAL).
- **Session 11 work (2026-05-01):** Phase 5 of back-to-basics-plan COMPLETE + MERGED + PUSHED. Re-rendered 12 V1.2 wild scans (catalog entries 16-27) through calibration v2 + template-side derivation. 12 bundles mutated (`phase_3_advisory.scorecard_hints` replaced with `compute_scorecard_cells_v2()` output + `shape_classification` added). Comparison doc at `docs/calibration-rebuild-rerender-comparison.md`. 0 verdict shifts; 10 cell shifts; 3 redundant overrides cleared (Kronos Q2, ghostty Q1, freerouting Q4 — rule-driven now). Out-of-scope entries 1-15 unchanged: 1-11 V2.4-era V1.1 schema, 12-14 Step G V1.1 phase_4, 15 markitdown empty phase_4 cells. Owner decision (W) defer on gate 6.3 backlog (7 cells across 6 entries, mostly Q3 FALLBACK regression).
- **Session 10 work (2026-05-01):** Phase 3 (calibration v2 module) + Phase 4 (template-side derivation) of back-to-basics-plan. Phase 3: classify_shape + evaluate_q1-q4 + compute_scorecard_cells_v2 in `docs/compute.py`; 5/10 cells rule-driven (~50% override reduction). Phase 4: 3 helpers in `docs/render_helpers.py` (derive_repo_vitals/coverage_detail/pr_sample) wired into 6 template partials with override-fallback pattern. Spec deviations at `docs/calibration-impl-notes.md`.
...
- **Tests:** **609/609 passing** (565 session-10 baseline + 22 session-12 Phase 7 in `tests/test_render_simple.py` covering 3 representative bundles + minimal-bundle fallback + self-containment + HTML/MD parity + pure-function unit tests + 22 net new across other suites in sessions 11/12).
