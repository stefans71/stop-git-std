...
- **HEAD:** `edf8f57` on origin/main (session 6 close, freerouting wild scan). Tree clean.
- **Session 6 commits** from `be9a1c0` (session 5 close) →: `9f8023f` (Kronos catalog entry 17) → `06ef4b7` (kamal catalog entry 18 + V13-1 telemetry trigger fired) → `5f3984d` (V13-1 resolution: override_reason enum 5→7 values + 4 existing-override relabels) → `d803faf` (V1.2.x signal widening: +q1_has_ruleset_protection + q2_oldest_open_security_item_age_days + harness deserialization regex widened) → `baf5dc9` (D-6 severity-distribution comparator shipped per Operator Guide §8.8.7) → `6f98fb2` (Xray-core entry 19) → `02be2ef` (browser_terminal entry 20) → `5587560` (wezterm entry 21) → `a52ba7b` (QuickLook entry 22) → `b8be939` (kanata entry 23) → `edf8f57` (freerouting entry 24).
- **Tests:** 385/385 passing (342 session-5 baseline + 2 V13-1 override positive tests + 13 V13-1 helper tests + 4 harness pickle-regex tests + 23 D-6 severity-comparator tests + 1 signal-count update).
- **Parity sweep:** All V1.2 wild scan MD+HTML pairs clean; post-V13-1 enum migration re-rendered the 3 existing scans.
- **Repo↔package validator:** byte-identical (no package change this session).
- **CSS:** 824 lines (unchanged).
- **Catalog:** **24 entries** — 11 `v2.4` + 13 `v2.5-preview`: 3 Step G validation (12-14) + **9 V1.2-schema wild scans** (entries 16–24). Latest: freerouting entry 24 with confirmed Java-deserialization RCE class (`BasicBoard.java:108-110` ObjectInputStream.readObject on user-loaded files).
- **Schema: V1.2 (LIVE)** — unchanged from session 5; `override_reason_enum` expanded 5→7 values per V13-1 owner directive (+`signal_vocabulary_gap`, +`harness_coverage_gap`).
- **Compute:** `SIGNAL_IDS` expanded 23→25 per V1.2.x (+`q1_has_ruleset_protection`, +`q2_oldest_open_security_item_age_days`); `OVERRIDE_REASON_ENUM` 5→7 values; new helpers `derive_q1_has_ruleset_protection` + `derive_q2_oldest_open_security_item_age_days` for scan drivers.
- **Harness:** `dangerous_primitives.deserialization` regex widened to include `pickle.loads?`, `marshal.loads?`, `joblib.load`, `dill.loads?` (closes Kronos Q4 gap; surfaced freerouting's real 35-file Java deserialization surface correctly with zero false positives).
- **New tooling this session:** `docs/compare-severity-distribution.py` (D-6 gate-6.2 automation — 23 tests; all 3 Step G pairs pass with 4/4, 5/5, 9/9 matches).
- **New docs this session:** `docs/v13-1-override-telemetry-analysis.md` (V13-1 split analysis + owner-directive decision) + `docs/v12-wild-scan-telemetry.md` (9-scan cross-analysis — MUST READ alongside this file for the override-enum distribution, harness-patch candidates, and V13-3 progress).
- **Gate 6.3: override-explained** (unchanged from session 5 definition). Across 7 overrides in 9 V1.2 wild scans: `signal_vocabulary_gap` × 4 (57%, modal), `harness_coverage_gap` × 2 (29%), `threshold_too_strict` × 1 (14%). 4 of 7 enum values unexercised.
- **Zero-override streak observed** at 3 consecutive (wezterm → QuickLook → kanata), broken by freerouting (Java deserialization finding exceeds compute signal coverage).
- **V13-3 progress: 9 of 11 V1.2 wild scans** accumulated (2 more to trigger comparator-calibration analysis).
- **V2.5-preview: PRODUCTION-CLEARED** (unchanged from session 5; harness is the Phase 1 standard).
...
- **V1.3 watchpoints (deferred per CONSOLIDATION §8 / V13-*)** — triggered from 2026-04-20 V1.2 landing onward:
  ...
  - **V13-3 11-scan comparator calibration analysis.** Trigger: V1.2 schema frozen + telemetry exists + ≥11 V1.2 scans. **9 of 11 V1.2 wild scans accumulated (entries 16–24)** — 2 more needed. See `docs/v12-wild-scan-telemetry.md` for the 9-scan cross-analysis so far.
  - **V13-4 `community_norms_differ` enum expansion** (DeepSeek R3 preserved dissent). Trigger: ≥3 V1.2 overrides with `missing_qualitative_context` citing community norms. **Status: 0 firings post-V13-1 relabel** (none of the 9 scans' overrides cite community norms; the split into `signal_vocabulary_gap` + `harness_coverage_gap` absorbed what was previously ambiguous). V13-1 analysis §4 predicted this; the V13-4 trigger may never fire.
  ...
  - **V12x-5 Deserialization auto-fire for Q4** (new, high-priority) — `q4_has_critical_on_default_path = True` when `dangerous_primitives.deserialization.hit_count >= N` on file-loading tools. Would collapse Kronos Q4 + freerouting Q4 overrides. Trigger: next V1.2.x patch cycle.
  - **V12x-6 Multi-ecosystem manifest parsing** (new, high-priority) — Maven/Gradle/Cargo/gomod/Bundler/NuGet parsing + OSV-query. Closes the F5/F6-class findings in 6 of 9 V1.2 scans (Go, Ruby, Rust × 2, C#, Java).
  - **V12x-7 Install-doc URL TLD-deviation detection** (new, medium-priority) — would auto-surface browser_terminal's F0 typosquat without Phase 4 override.
  - **V12x-8 Language-specific distribution-channel detection** (new, medium-priority) — 7 of 9 scans surface fictitious or missing channels.
  - **V12x-9 Dangerous-primitives regex tuning for Rust serde / bundled minified JS** (new, low-priority) — false positives on 3 scans.
  - **V12x-10 Silent-fix pattern telemetry** (new, informational) — 9 of 9 V1.2 scans show 0 published GHSA advisories. May warrant a Q3 rubric adjustment in V1.3 once pattern is confirmed at 11+ scans.
