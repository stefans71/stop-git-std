...
- **HEAD:** `5f5d1cd` on origin/main (session 5 close, ghostty wild scan). Tree clean.
- **Session 5 commits** from `4512084` (session 4 close) →: `a0e370e` (V1.2 design frozen — board review 3/3 SIGN OFF) → `2c5aca0` (V1.2 implementation landing) → `5f5d1cd` (ghostty catalog entry 16).
...
- **Parity sweep:** 18/18 MD+HTML pairs clean (13 v2.4 catalog + 5 v2.5-preview).
- **Repo↔package validator:** byte-identical (no package change this session).
- **CSS:** 824 lines (unchanged).
- **Catalog:** 16 entries — 11 `v2.4` + 5 `v2.5-preview`: 3 Step G validation (entries 12-14: zustand-v3, caveman, Archon) + 2 wild (entry 15 markitdown, entry 16 **ghostty** — first V1.2-schema scan + first override-explained exercise end-to-end).
- **Schema: V1.2 (LIVE as of 2026-04-20).** Harness-canonical shapes + names; transformer deleted; `phase_3_advisory.scorecard_hints` new top-level; `phase_4_structured_llm.scorecard_cells` authoritative with `{rationale, edge_case, suggested_threshold_adjustment, computed_signal_refs, override_reason}`; 23-row frozen signal ID vocabulary (`compute.SIGNAL_IDS` with `q1_`/`q2_`/`q3_`/`q4_`/`c20_` prefixes); 5-value `override_reason` enum frozen; `agent_rule_files.total_bytes` dropped. Board record: `docs/External-Board-Reviews/042026-schema-v12/CONSOLIDATION.md`.
- **Gate 6.3: override-explained** — validator (`docs/validate-scanner-report.py --form`) enforces rationale ≥50 chars + `computed_signal_refs` non-empty with all refs resolving against `compute.SIGNAL_IDS` + `override_reason` in enum, only when Phase 4 color differs from Phase 3 advisory color. **Proven end-to-end** via ghostty entry 16 Q1 (advisory red → Phase 4 amber with `missing_qualitative_context` + 7 signal refs).
- **V2.5-preview: PRODUCTION-CLEARED 2026-04-20** (fired by markitdown entry 15). Phase 1 harness-driven (`docs/phase_1_harness.py`); V1.2 schema accepts harness output natively (no transformer, no sidecar bridge). Spec at `docs/phase-1-checklist.md`.
- **Migration:** `migrate-v1.1-to-v1.2.py` (repo-root) — idempotent; all 3 active fixtures (`zustand-form.json`, `caveman-form.json`, `archon-subset-form.json`) migrated clean. `step-g-failed-artifact` fixtures stay V1.1 (regression witnesses per CONSOLIDATION §5 Item B).
- **Cross-repo FrontierBoard SOP** (pushed to stefans71/FrontierBoard main as `e01303a`): pre-archive dissent audit gate mandatory per SOP §4. Canonical at `/root/.frontierboard/FrontierBoard/docs/REVIEW-SOP.md`.
...
- **V1.3 watchpoints (deferred per CONSOLIDATION §8 / V13-*)** — triggered from 2026-04-20 V1.2 landing onward:
  - **V13-1 Override-pattern telemetry log** (CSV of `cell, advisory_color, llm_color, rationale_first_line, override_reason`). Trigger: 3 V1.2 scans shipped. Currently 1/3 (ghostty entry 16).
  - **V13-2 Renderer Phase-1 richness expansion** (surface nested `dangerous_primitives` per-family, `self_merge_count`, `dependabot_config` schedule in finding cards). Trigger: V1.2 round-trip + determinism pass clean on all regenerated fixtures — both met 2026-04-20 session 5.
  - **V13-3 11-scan comparator calibration analysis.** Trigger: V1.2 schema frozen + telemetry exists + ≥11 V1.2 scans collected.
  - **V13-4 `community_norms_differ` enum expansion** (DeepSeek R3 preserved dissent). Trigger: ≥3 V1.2 overrides with `missing_qualitative_context` citing community norms.
  - **V13-5 SD2 auto_load_tier C3 rebinding** (`domain=='agent'`). Own review cycle.
  - **V13-6 markitdown-as-4th-fixture decision.** Trigger: markitdown re-scanned under V1.2.
  - **V13-7 OQ-9 sidecar reconsideration** if harness gathers data not representable in schema. Trigger: any harness feature add that fails B1 round-trip.
  - **V12x-1..V12x-4 in-line V1.2.x patches** (additive only, no schema bump): gather 4 remaining repo_metadata fields in harness if not already done; rename `q4_has_*_on_*_path` signals if ordering assumption fails; add `c_top_contributor_share` unthresholded; add more supporting signals per telemetry.
- **D-6 Automated severity distribution comparison script** — POST-STEP-G IMMEDIATE per Operator Guide §8.8.7. Manual check in gate 6.2 is sufficient for the 3-target Step G scope but does not scale to broader deployment. Script is the natural next step post-Step-G success; NOT indefinitely deferred. Trigger: before production V2.5-preview scan beyond the 3 validation shapes (markitdown + ghostty already ran without it — this is accumulating debt).
