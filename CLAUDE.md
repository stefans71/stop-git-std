...
As of 2026-04-20 session 4: Scanner prompt V2.4, Operator Guide V0.2 (§8.8 implements Step G + production-clearance + V1.2), **scan-schema.json at V1.2**, 342/342 tests passing, 15 catalog entries (11 V2.4 + 4 `v2.5-preview`: 3 Step G validation + 1 wild), Phase 7 renderer Steps A-F complete, Phase 1 harness shipped. **V2.5-preview PRODUCTION-CLEARED 2026-04-20.** Step G 3/3 validation + first wild scan on microsoft/markitdown (Python monorepo, 112k stars — catalog entry #15) cleared all 7 applicable gates. Phase 1 automation: `docs/phase-1-checklist.md` + `docs/phase_1_harness.py` + `tests/test_phase_1_harness.py` (commit `4ad4cf6`). **Schema V1.2 DESIGNED AND SHIPPED 2026-04-20:** design via co-scheduled D-7+D-8 board review (3 rounds + owner directives; 3/3 SIGN OFF; 31-item dissent audit with zero silent drops; archive `docs/External-Board-Reviews/042026-schema-v12/CONSOLIDATION.md`); implementation landed schema bump + harness additions + `compute.SIGNAL_IDS` (23-entry frozenset) + validator `--form` mode with gate 6.3 override-explained + scorecard partial path migration + `migrate-v1.1-to-v1.2.py` idempotent migration + 23 new V1.2 tests. Transformer deleted. Sessions 3+4 commits on top of `e267805`: SF1 archive + Phase 1 patches + U-11 + 3 Step G scans + session-close + Phase 1 harness + markitdown wild scan + schema-V1.2 board review + V1.2 implementation.
