...
**Last back-to-basics-plan event: 2026-05-01 session 10 — Phase 3 implementation COMPLETE.** Calibration v2 module landed (`docs/compute.py` `classify_shape()` + `evaluate_q1/q2/q3/q4` + `compute_scorecard_cells_v2()`). 565/565 tests passing (414 baseline + 151 net new across `tests/test_classify_shape.py`, `tests/test_calibration_rules.py`, `tests/test_validator_v2_rule_id.py`, `tests/test_calibration_regression.py`). All 5 Phase 3 carry-forwards from CONSOLIDATION §5 addressed. Override-reduction outcome: 5 of 10 known cells now rule-driven (~50% reduction → hits design hard-floor ≤5/12 ~42%). Phase 3 **merged + pushed to origin/main at `9d33799`** (no-ff merge of `chore/calibration-rebuild-impl`; feature branch deleted). Phase 3 spec deviations consolidated at `docs/calibration-impl-notes.md` (9 sections, all deviations from `docs/calibration-design-v2.md` documented with rationale). Post-merge pointer cleanup landed at `6657e59` (current HEAD on `origin/main`). **Phase 4 (mechanical reformatting moves to template-side) is the next active work, not yet started.** Recommended branch: `chore/template-side-derivation` from main. See plan §Phase 4 for deliverables.
...
As of 2026-04-30 session 8: Scanner prompt V2.4, Operator Guide V0.2, **scan-schema.json LIVE at V1.2** (with V13-1 enum expansion + V1.2.x signal widening + **V13-3 harness patches**), **414/414 tests passing**, **27 catalog entries** (11 V2.4 + 16 v2.5-preview: 3 Step G validation + **12 V1.2-schema wild scans** — entries 16–27). **12 V1.2 wild scans (V13-3 CLOSED; follow-up cadence active)** cover an increasingly diverse shape space: terminal emulators (ghostty Zig + wezterm Rust), Python ML (Kronos), Python monorepo (markitdown), Ruby deploy orchestrator (kamal), Go network proxy (Xray-core), browser extension + Go native host (browser_terminal), C# Windows shell extender (QuickLook), Rust keyboard daemon (kanata), Java PCB autorouter (freerouting), embedded ESP32 IoT firmware (WLED), reverse-engineered WhatsApp Web library (Baileys), Claude Code skills/plugin (mattpocock/skills — entry 27, session 8).
...
**Override telemetry across 12 V1.2 wild scans (n=12)**: 10 total overrides. Enum distribution: `signal_vocabulary_gap` × 6 (60%, modal — ghostty Q1, kamal Q1, Kronos Q2, freerouting Q4, WLED Q4, Baileys Q4); `harness_coverage_gap` × 2 (Kronos Q4, browser_terminal Q4); `threshold_too_strict` × 1 (Xray-core Q2); **`missing_qualitative_context` × 1 (skills Q2 — first use of this enum value at any n; session 8)**. 3 zero-override scans (wezterm + QuickLook + kanata). 12/12 silent-fix pattern (0 published GHSA advisories across all 12). TWO concretely-documented disclosure-handling failures: WLED issue #5340 → GHSA-2xwq-cxqw-wfv8 (74 days silent) + Baileys PR #1996 stale-closed after 148 days with maintainer-promised replacement never materialized. **Entry 27 telemetry note**: first `missing_qualitative_context` fire is a taxonomy-strain event candidate — n=11-bounded inference "the V13-1 split was correct and complete" no longer holds at n=12; sample-floor degeneracy (1 lifetime merged PR) is a live use case for the catchall enum value that V13-3 C9 had proposed pruning. V12x-15 (sample-floor signal for Q2 advisory) candidate added. Full cross-scan analysis at **`docs/v12-wild-scan-telemetry.md`** (MUST READ for any V1.2-era decision).

**V13-3 CLOSED 2026-04-20** — 042026-v13-3-comparator-calibration board review, 2/3 R3 AGREE + owner directive OD-4 (Codex R3 unavailable, R2 positions consistent with R3 resolutions; Codex assigned code-review gate on staged V1.2.x implementation diff). **V1.2.x landed:** C2 language-qualifier regex (drops bare `deserialize` keyword — suppresses WLED-class ArduinoJson FPs), C5 Q4 auto-fire from unsafe-deserialization hits + tool-loads-user-files condition, C18 `derive_tool_loads_user_files()` canonical helper, C20 dry-run FP test at `docs/v13-3-fp-dry-run.md` (0 FPs on 11 bundles). **Frozen analysis:** `docs/v13-3-analysis.md`. **V1.3 deferred:** C6 firmware CORS compound, C7 reverse-eng shape with REJECT fallback, C9 prune `missing_qualitative_context` with historical-hold, C17 multi-ecosystem manifest parsing, V13-4 `community_norms_differ` enum (N=15 or first Q3 override fire). **Follow-up cadence:** G4-broadened — N=25 OR any of 6 taxonomy-strain events. Full record at `docs/External-Board-Reviews/042026-v13-3-comparator-calibration/CONSOLIDATION.md`.

Phase 1 is **harness-driven**; V1.2 schema accepts harness output natively. Session 8 single commit on top of session 7: `2aa59bf` (catalog entry 27 mattpocock/skills — Caution; Q2 missing_qualitative_context override). No code changed in session 8 (data + docs only). Session 7 baseline at `9b5e5d4` preserved.
...
- **If you need the cross-scan V1.2 telemetry analysis** → `docs/v12-wild-scan-telemetry.md` (11-scan override-enum distribution + harness-patch candidates + **V13-3 TRIGGER FIRED**)
