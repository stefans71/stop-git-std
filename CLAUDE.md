...
As of 2026-04-20 session 7: Scanner prompt V2.4, Operator Guide V0.2, **scan-schema.json LIVE at V1.2** (with V13-1 enum expansion + V1.2.x signal widening + **V13-3 harness patches**), **404/404 tests passing**, **26 catalog entries** (11 V2.4 + 15 v2.5-preview: 3 Step G validation + **11 V1.2-schema wild scans** — entries 16–26). **11 V1.2 wild scans (V13-3 CLOSED)** cover an increasingly diverse shape space: terminal emulators (ghostty Zig + wezterm Rust), Python ML (Kronos), Python monorepo (markitdown), Ruby deploy orchestrator (kamal), Go network proxy (Xray-core), browser extension + Go native host (browser_terminal), C# Windows shell extender (QuickLook), Rust keyboard daemon (kanata), Java PCB autorouter (freerouting), embedded ESP32 IoT firmware (WLED), reverse-engineered WhatsApp Web library (Baileys).
...
**Override telemetry across 11 V1.2 wild scans**: 9 total overrides; `signal_vocabulary_gap` strongly modal at 67% (6 of 9 — 3 consecutive Q4 signal_vocabulary_gap overrides on freerouting + WLED + Baileys); 3 zero-override scans (wezterm + QuickLook + kanata). 11/11 silent-fix pattern (0 published GHSA advisories across all 11). TWO concretely-documented disclosure-handling failures: WLED issue #5340 → GHSA-2xwq-cxqw-wfv8 (74 days silent) + Baileys PR #1996 stale-closed after 148 days with maintainer-promised replacement never materialized. Full cross-scan analysis at **`docs/v12-wild-scan-telemetry.md`** (MUST READ for any V1.2-era decision).
...
Phase 1 is **harness-driven**; V1.2 schema accepts harness output natively. Session 7 commits on top of `f4f3332` (WLED) → `c558b9f` (Baileys) → V13-3 landing commit. 19 new V13-3 tests (`TestV13_3_DeriveToolLoadsUserFiles` × 10, `TestV13_3_Q4AutoFireFromDeserialization` × 7, `test_v13_3_c2_arduinojson_deserializejson_suppressed`, `test_v13_3_c2_unsafe_specific_still_matches`).
