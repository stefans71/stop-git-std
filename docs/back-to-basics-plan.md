...
**Status:** **PLAN COMPLETE.** All 8 phases of the back-to-basics calibration rebuild (0 through 7) landed on `origin/main`. **30 catalog entries; 15 V1.2 wild scans; 652/652 tests passing.** Latest entry: onecli/onecli @ `fa6468e` (session 16) — credential-vault for AI agents, TS+Rust monorepo with MITM HTTPS gateway; Caution; **first wild V1.2 scan to fire Q4 `signal_vocabulary_gap` on a design-risk dimension** (MITM-by-design / CA-cert-trust posture).

**Next-session action — run a wild scan or pick a follow-up.** Pick a repo, say "scan `<owner/repo>`", follow the wizard. Wizard defaults: Q1 = long-form MD + Simple Report HTML; Q3a = V2.5-preview. The pipeline is end-to-end validated (multica entry 28 + impeccable entry 29 + onecli entry 30 all exercised cleanly). After the scan completes, present the post-scan options (CLAUDE.md "After the scan completes" section).

**Taxonomy-strain watch (read before next scan):** V13-3 broadened cadence at **15/25** toward N=25 trigger; **3/6 taxonomy-strain events fired** (skills entry 27 + multica entry 28 + impeccable entry 29 — all `missing_qualitative_context` Q2). **onecli entry 30 fired Q4 `signal_vocabulary_gap` (a different override category) — not a 4th `missing_qualitative_context` strain event.** Driver tally unchanged: sample-floor degeneracy × 1 (skills); close-rate counter-signal × 2 (multica + impeccable). **If a fourth `missing_qualitative_context` fire arrives with a genuinely third-distinct semantic driver**, escalate to board review immediately rather than wait for N=25 — the catchall is then genuinely unbounded, not just under-split.

**Shape-classifier strain noted (logged):** impeccable (entry 29) misclassified as `library-package` — real shape is multi-harness agent-skills-collection (468 skill files across 13 harness directories). Same Step 2 too-strict gate that affects skills entry 27 (caught only by the secondary Markdown-canonical branch because skills is markdown-primary; impeccable's primary_language is JavaScript so neither branch fires). **V12x-18 backlog row landed in commit `d7f3d2a`** (telemetry §8 + plan long-running follow-ups below) with the signal-ratio fix design + F5-harness-recursion dependency.

**Q4 design-risk signal gap noted (logged 2026-05-06):** onecli (entry 30) fired the first wild V1.2 Q4 `signal_vocabulary_gap` on a design-risk dimension — Phase 3 advisory amber FALLBACK is factually correct on installer signals but does not capture the architectural-trust-surface implication of MITM-by-design / CA-cert-install postures. The gateway requires installing a generated CA cert as a trusted root → all of an agent's outbound HTTPS traffic flows through the gateway in cleartext, not just calls to APIs configured for credential injection. Compromise of the gateway host yields visibility into all agent traffic, not just stored API keys. **V12x-19 backlog row landed in commit `a8f0e97`** (telemetry §8 + plan long-running follow-ups below) with a `q4_design_requires_root_trust` signal proposal coupled with shape × maturity modifiers (agentic-platform + < 6mo + solo-maintained + design_requires_root_trust → red).

**`web-application` classifier branch — still a stub.** `docs/compute.py::classify_shape()` Step 9 (line 1469) is a deferred stub awaiting first-hit per the established "first-hit principle" used for `agentic-platform` Step 7.5 (which fired for multica entry 28). No V1.2 wild scan has cleanly hit the web-application shape yet (postiz-app and Archon are V2.4-era; multica fired agentic-platform first). Candidate signal set sketched in the in-code comment: docker-compose.yml present + single-ecosystem npm + no agentic-platform multi-stack signal. Activate when the next docker-compose-only npm web-app lands a wild scan.

**Phase 4 token cost is the next major efficiency target (~150-180k tokens per scan).** Current authoring of `phase_4_structured_llm.findings.entries[]` + `phase_5_prose_llm.per_finding_prose.entries[]` is fully LLM-driven; the impeccable scan (entry 29) burned ~150-180k tokens at Phase 4 alone (6 findings × prose paragraphs + threat models + action steps + evidence + scorecard rationale). **The architectural answer is `compute_findings(form)` from calibration-design v2 §7** (`docs/calibration-design-v2.md` lines 430-456): a finding-pattern catalog where each detectable signal pattern maps to a finding template (kind + severity + threat_model + action_hint), with the LLM reduced to (a) authoring narrative-only synthesis findings that don't claim severity, and (b) writing the per-finding prose paragraphs. Original design framed this as **Phase 1.5** (after Phase 5 re-render shows cell calibration works — that re-render landed cleanly in session 12). Phase 5 has long since validated cells; Phase 1.5 is now the highest-leverage next investment. Entry-29 + entry-28 finding-bundle data is fresh material for the finding-pattern audit that would precede Phase 1.5 design.
...
| **Session 15 — pbakaus/impeccable wild scan (entry 29)** | 14th V1.2 wild scan. Multi-harness agent-skills (468 skill files × 13 harness directories) + npm CLI + Chrome extension. Verdict Caution. **First scan with two `missing_qualitative_context` overrides in a single form** (Q2 close-rate counter-signal — same driver as multica; Q4 channel-count denominator misleading — 23/24 channels are test fixtures). Open #92 build-time RCE in build scripts via `new Function()`; 3-hop install supply chain via vercel-labs/skills. **Shape misclassified `library-package`** (real shape is agent-skills-collection — Step 2 too-strict gate; backlog item V12x-18 added). | Commit `d7f3d2a`; bundle `docs/scan-bundles/impeccable-444e4ac.json`; `AUDIT_TRAIL.md` session 15 |
| **Session 16 — onecli/onecli wild scan (entry 30)** | 15th V1.2 wild scan. **First credential-vault / agent-secrets-broker shape entry.** TS+Rust monorepo: Rust gateway (`apps/gateway`) does MITM HTTPS interception via rcgen-generated CA cert; AES-256-GCM via `ring 0.17`; Bitwarden integration via `ap-*` v0.9.0. Solo (guyb1 85%); 2,093 stars; ~2 months old. Active branch ruleset on main but no required-reviewer rule (0.0% formal review on 208 PRs). Verdict Caution. **First wild V1.2 Q4 `signal_vocabulary_gap` on a design-risk dimension** — MITM-by-design / CA-cert-trust posture. Shape correctly classified `agentic-platform` (medium confidence) — second hit on Step 7.5 after multica entry 28; §6 shape table 1→2. New backlog item V12x-19 — `q4_design_requires_root_trust` signal. | Commit `a8f0e97`; bundle `docs/scan-bundles/onecli-fa6468e.json`; `AUDIT_TRAIL.md` session 16 |

**Tests:** 652/652 passing. **Catalog:** 30 entries; entries 16-30 have Simple Reports (15 of 30); entries 1-15 do not (V2.4-bundle adapter is a follow-up).
...
| **V12x-19 — Q4 design-risk signal: `q4_design_requires_root_trust` for MITM-by-design / CA-cert-install / kernel-module / system-wide-LD_PRELOAD postures.** | When the next wild scan hits a comparable architecture (credential proxy, agent secrets broker, eBPF/kernel daemon, system-wide LD_PRELOAD library) — OR if onecli is re-scanned at a future SHA. | onecli (entry 30) is the first fire — Phase 3 advisory FALLBACK is factually correct on installer signals (no pinned channels, no artifact verification) but does not capture the architectural-trust-surface implication of designs that require trusting a generated CA cert. Once the agent trusts the CA, every HTTPS call the agent makes is decrypted by the gateway — not just calls to APIs configured for credential injection. Couple with shape × maturity modifiers: agentic-platform + < 6mo + solo-maintained + design_requires_root_trust → red. Detection sources: README pattern matching for "install our CA" / "trusted root" / "MITM" / "kernel module" / "root daemon" / "LD_PRELOAD"; presence of `rcgen` / `mkcert` / CA-related crypto crates in Cargo.toml; presence of system-wide install-script paths (`/etc/`, `/usr/local/share/ca-certificates/`). Tracked in telemetry §8 V12x-19. |
| **`web-application` classifier branch — still a stub** | First V1.2 wild scan on a docker-compose-only npm web-app (no Go/Python/Ruby backend tier) | `docs/compute.py::classify_shape()` Step 9 (line 1469) is a deferred stub. No V1.2 wild scan has cleanly hit this shape yet. Candidate signal set sketched in the in-code comment: docker-compose.yml present + single-ecosystem npm + no agentic-platform multi-stack signal. Same first-hit pattern that landed agentic-platform Step 7.5 from multica (entry 28). |
| **Phase 1.5 / `compute_findings()` — Phase 4 token-cost reduction** (NEXT MAJOR EFFICIENCY TARGET) | Owner schedules a finding-pattern audit, OR a future scan's Phase 4 token cost becomes a budget constraint, OR enough catalog material accumulates (n=14 V1.2 wild scans = solid audit base) | Phase 4 finding-authoring currently burns ~150-180k tokens per scan (impeccable entry 29 datapoint; consistent with prior V2.5-preview scans). The architectural answer is **`compute_findings(form)`** from `docs/calibration-design-v2.md` §7 (lines 430-456): a finding-pattern catalog mapping detectable signal patterns to finding templates (kind + severity + threat_model + action_hint). LLM reduced to (a) authoring narrative-only synthesis findings without severity claims, and (b) writing per-finding prose paragraphs. Original design framed this as Phase 1.5 (after Phase 5 re-render validated cells — that landed in session 12). Audit scope: classify the existing 14 V1.2 wild-scan findings (entries 16-29) into pattern buckets to see what's programmaticizable. Expected outcome per design: cell-calibration goes from ~95% rule-driven to combined ~70%→90% programmatic on scoring overall. Needs its own audit + design + board review cycle. |
