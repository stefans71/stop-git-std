...
> - **[S8-3] Exhibit rollup.** When a section has 7+ similar-severity items, roll into themed exhibit panels (vuln/govern/signals) using `<details>/<summary>`. In MD use `<details>` blocks.
...
> - **[S8-6] Section status pills + section-action on every flagged section.** Summary count line (`⚠ N Warning · ℹ N Info · ✓ N OK`) + `**Your action:**` block. In MD use blockquote lines under the heading.
