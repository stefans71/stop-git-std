...
| 2026-05-01 | `050126-calibration-rebuild/` | Phase 2 of `docs/back-to-basics-plan.md` — calibration design v2 board review. Rule-table rebuild for the 4 scorecard cells (Q1+Q3+Q4); 9-category shape classifier as modifier; 10 specific rules (RULE-1..RULE-10); template-map cell short_answer language; cells-only scope (finding-severity rule-driving deferred to Phase 1.5). 16 owner directives applied across 3 rounds (8 R1 + 7 R2 + 1 R3 pre-archive doc fix). | R1-R3 | **3-of-3 R3 SIGN OFF** — Pragmatist clean SIGN OFF, Codex + DeepSeek SIGN OFF WITH NOTES (deferrals to Phase 3 §10 "must resolve" sub-section). DeepSeek moved DISSENT (R1) → SIGN OFF WITH NOTES (R2/R3). 26-item dissent audit · zero silent drops · zero preserved-live blocking dissents (Codex R3 preserved Q2-doc-gap dissent raised-and-resolved-pre-archive via §5 Q2 sub-section). |
