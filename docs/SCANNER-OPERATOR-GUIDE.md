...
> **V2.5-preview — production-cleared as of 2026-04-20 after first wild scan on microsoft/markitdown (catalog entry #15).**
...
> - Production-clearance trigger fired 2026-04-20: first wild scan on Python monorepo shape (microsoft/markitdown, 112k stars — outside the 3 Step G validation shapes) cleared all 7 applicable gates using harness-gathered Phase 1 data. Gate 6 is N/A on wild scans (no V2.4 comparator); gates 1, 2, 3, 4, 5, 6.6, 7 all clean.
> - **Phase 1 is now harness-driven.** `docs/phase_1_harness.py` (shipped commit `4ad4cf6`) implements V2.4 prompt Steps 1-8 + A/B/C end-to-end: gh api + OSSF Scorecard + osv.dev + gitleaks + package registries (PyPI/npm/crates.io/RubyGems) + tarball extraction + local grep + README paste-scan. Run: `python3 docs/phase_1_harness.py <owner/repo> --out FORM.json`. See `docs/phase-1-checklist.md` for the canonical step→field mapping. 30 tests under `tests/test_phase_1_harness.py`.
...
> - **D-8 open:** harness produces richer fields than V1.1 schema accepts natively; current bridge emits a sidecar (`phase-1-raw-full.json`) + V1.1-compliant transformer. V1.2 schema update will accept harness output directly. See §8.8.7 ledger.
> - **Last reviewed: 2026-04-20.** Step G acceptance confirmed (commits `2c13324`, `ed68fae`, `be56935`). Production clearance fired on markitdown wild scan. SF1 scorecard-calibration board: `docs/External-Board-Reviews/042026-sf1-calibration/CONSOLIDATION.md`. Step G execution board: `041926-step-g-execution/`.
