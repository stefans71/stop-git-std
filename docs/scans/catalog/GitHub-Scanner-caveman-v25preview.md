...
**Investigated:** 2026-04-20 | **Applies to:** main @ `c2ed24b3e5d412cd0c25197b2bc9af587621fd99` · v1.6.0 | **Repo age:** 0 years | **Stars:** 32,844 | **License:** MIT

> A twelve-day-old Claude Code plugin that shipped real local-privilege-escalation vulnerabilities before it got to a thousand stars — and fixed them without an advisory.
...
| Commit pinned | `c2ed24b3e5d412cd0c25197b2bc9af587621fd99` |
...
Code in v1.6.0 is sound. But every install path fetches live main, and there is no branch protection on main, no rulesets, no CODEOWNERS (F0). A single compromised maintainer credential ships code to every Claude Code session on the next install.

### Version · Version · Historical installs · v1.0.0 – v1.5.1 — **Critical — Upgrade required**

A local-privilege-escalation symlink vulnerability shipped for 11 days. No advisory was issued (F5). If you installed before 2026-04-15, upgrade now — do not skip. The F0 governance gap applies to historical installs too.
...
1. **Vulnerability** — Symlink-based file-clobber in hooks. v1.0.0–v1.5.1 affected. Fixed in v1.6.0.
2. **Disclosure · F5** — Unadvertised — release notes only, no GitHub Security Advisory.
3. **Response lag** — Security PR sat open 5 days before merge.
...
1. **Review rate · F11** — 15% formal (5/33), 36% any-review. Security PRs #70/#71 zero review.
2. **Batch merge** — v1.6.0 was a 16-PR batch merged in a 23-minute window, hiding the security fix.
3. **Distribution** — Every install channel fetches live main or unpinned artifacts.
4. **Governance · F14** — No branch protection on main. No CODEOWNERS. Dependabot disabled.

</details>

<details>
<summary><strong>✅ Positive signals — code quality, scope, maintainer legitimacy (3 findings)</strong>
<br><em>v1.6.0 code is defensive, dep surface minimal, maintainer is a 4-year account</em></summary>

1. **Code quality · F16** — v1.6.0 hardening code is thorough: O_NOFOLLOW + atomic rename + whitelist-validated reads + size caps.
...
*Continuous · Since repo creation · → Maintainer-side: enable branch protection on main, require 1+ approver, add .github/CODEOWNERS. User-side: pin installs to a tagged release, not main.*
...
**What this means for you.** When you install caveman, you are trusting JuliusBrussee's account security as tightly as you'd trust your own laptop's password. There is no automated gate that would catch 'maintainer pushed a bad change' before it reaches your next install, and the hook files run on every Claude Code session. The blast radius of a single credential compromise is every current and future user — 32,844 stars worth.
...
| Releases in last 30 days | ⚠ 10 (active flow → C20 Critical threshold) |
| Active executable surface | Hooks run in every Claude Code session |

**How to fix.** GitHub *Settings → Branches → Add branch protection rule* for `main`: require pull request reviews (1 approver minimum), require status checks, require branches up-to-date before merging, do not allow bypassing. Add `.github/CODEOWNERS` covering at minimum `hooks/`, `rules/`, `.github/workflows/`, and install scripts. At 32k stars this is basic hygiene.

### F5 — Warning · Process — Unadvertised symlink vulnerability — named in release notes, not in GitHub's advisory channel

*2026-04-04 – 2026-04-15 (11 days live) · Code fixed in v1.6.0 · Disclosure gap open · → If you installed v1.5.1 or earlier, upgrade to v1.6.0 now. New installers on v1.6.0 are fine — but ask the maintainer to publish a GitHub Security Advisory so past-users have a programmatic upgrade signal.*
...
**What this means for you.** Users who installed v1.0.0–v1.5.1 have no programmatic way to learn they should upgrade. The release note for v1.6.0 reads 'Hardening release: hook crash fixes + symlink-safe flag writes' but doesn't call out that every prior version had a live exploitable bug. The fix itself is good; the disclosure process is not.
...
| Meta | Value |
|------|-------|
| Affected versions | v1.0.0 – v1.5.1 (11 days live) |
| Reported by | tuanaiseo — [PR #70](https://github.com/JuliusBrussee/caveman/pull/70) |
| Fixed by | PR #70 + PR #71 (same reporter) |
| Reviewed by | ❌ Nobody |
| Time to merge | 5 days |
| Advisory published | ❌ None |
| SECURITY.md at root | ❌ No |

**How to fix.** Two-part fix. **Code:** already fixed in v1.6.0 (see F16). **Disclosure:** maintainer should publish a GitHub Security Advisory at https://github.com/JuliusBrussee/caveman/security/advisories for the symlink vulnerability affecting v1.0.0–v1.5.1, AND add `SECURITY.md` at repo root with a private reporting channel. Users of the affected versions then receive programmatic notification.

### F11 — Warning · Structural — Review rate: 15% formal (5/33), 36% any-review — security PRs #70/#71 had zero review activity

*Continuous · Since repo creation · → Only relevant if you rely on caveman for sensitive workflows. Ask maintainer to require one other approver on hook/workflow PRs.*

**What we observed.** The maintainer is doing thoughtful work — the v1.6.0 code is carefully hardened. But there's no second pair of eyes on changes before they ship to tens of thousands of developers' local machines. A cr

**What this means for you.** The maintainer is doing thoughtful work — the v1.6.0 code is carefully hardened. But there's no second pair of eyes on changes before they ship to tens of thousands of developers' local machines. A credential-compromise PR would not stand out against this baseline of unreviewed merges.

| Meta | Value |
|------|-------|
| PRs with formal review | 5 / 33 |
| Formal review rate | ❌ 15% |
| Any-review rate (includes informal) | ⚠ 36% |
| Security PRs with review | ❌ 0 of 2 (#70, #71) |

**How to fix.** In GitHub *Settings → Branches → branch protection rule for `main`*, check **Require a pull request before merging** and set **Required approving reviews** to 1. Combined with a `.github/CODEOWNERS` file (see F0 fix), this gates every merge through at least one other pair of eyes before it ships to users.

### F14 — Info · Observation — No branch protection rule on `main` (narrow fact — compounds F0 + F11)

*Current · API 404 at scan time · → No direct user action. Same maintainer-side fix as F0.*

**What we observed.** Not actively exploited right now, but combined with the 15% review rate and lack of a security advisory process, this is a governance gap that would compound any attacker who compromised the maintaine

**What this means for you.** Not actively exploited right now, but combined with the 15% review rate and lack of a security advisory process, this is a governance gap that would compound any attacker who compromised the maintainer's GitHub session or credentials.

| Meta | Value |
|------|-------|
| API response | 404 Not Found |
| Interpretation | No protection rule exists (definitive — 403 would indicate permissions) |

**How to fix.** Resolved by the same fix as F0: enable branch protection on main. F14 is the narrow single-fact observation (API 404 authoritative); F0 is the broader C20 Critical combining all four governance signals (branch protection, rulesets, rules-on-default, CODEOWNERS) plus the active-code-flow multiplier.

### F16 — OK · Positive — v1.6.0 security code is thorough and well-reasoned — defense-in-depth against symlink attacks

*v1.6.0 current · 2026-04-15 onward · → No action needed — positive signal. If you're looking at the v1.6.0 hook code, the safeWriteFlag and readFlag helpers are the part worth reading.*

**What we observed.** The safeWriteFlag and readFlag helpers correctly implement every defense expected for this class of vulnerability. The comments explicitly describe the attack scenarios being defended against. This is

**What this means for you.** The safeWriteFlag and readFlag helpers correctly implement every defense expected for this class of vulnerability. The comments explicitly describe the attack scenarios being defended against. This is good security engineering — the concern is not with the code but with the process that shipped it without an advisory.

| Meta | Value |
|------|-------|
| O_NOFOLLOW flag | ✅ Yes |
| Atomic temp+rename | ✅ Yes |
| Symlink refusal (parent + target) | ✅ Yes |
| Whitelist-validated reads | ✅ Yes |
| Read size cap | ✅ 64 bytes |
| File mode | ✅ 0o600 |
...
1. Windows PS1 hook variants not inspected in equal depth to Unix shell and Node.js files
2. 13 of 19 security-keyword PRs classified from metadata only — diffs not read in full
...
#### ★ Evidence 1 — v1.6.0 fixes a real symlink vulnerability present in v1.0.0–v1.5.1 (F5)
...
*Classification: Confirmed fact — vulnerability described in the reporter's own commit message. Fix in v1.6.0 adds O_NOFOLLOW and symlink refusal; before/after diff confirms the unprotected write in v1.5.1 and earlier.*

#### ★ Evidence 2 — Zero GitHub Security Advisories published for this repo (F5 disclosure gap)
...
*Classification: Confirmed fact — empty array response means zero advisories published over repo lifetime. No programmatic notification channel for the symlink vulnerability fixed in v1.6.0.*

#### ★ Evidence 5 — No branch protection on main — 404 authoritative (F0 + F14)
...
*Classification: Confirmed fact — 404 is authoritative here. The authenticated token has admin-read access; a permissions issue would return 403. 404 = no rule exists.*
...
```bash
gh pr list -R JuliusBrussee/caveman --state merged --limit 300 --json number,title,mergedAt | jq '[.[] | select(.mergedAt == "2026-04-15T12:50:16Z")] | length'
```
...
*Classification: Confirmed fact — 16 PRs share mergedAt 2026-04-15T12:50:16Z. The two security fix PRs (#70, #71) are among those 16, shipping alongside 14 unrelated PRs in a single release window.*

#### Evidence 4 — 85% of merged PRs have no formal review (F11)

```bash
gh pr list -R JuliusBrussee/caveman --state merged --limit 300 --json reviewDecision | jq '[.[] | select(.reviewDecision != null and .reviewDecision != "")] | length'
```
...
*Classification: Confirmed fact — 5 of 33 merged PRs have reviewDecision set = 15% formal review rate. Security PRs #70 and #71 specifically had zero review activity.*

#### Evidence 6 — Zero rulesets, zero rules on default branch, no CODEOWNERS (F0 C20 all four governance signals negative)
...
*Classification: Confirmed fact — no repo-level rulesets, no rules on main, CODEOWNERS absent in all four standard locations. Owner is a User account so no org-level rulesets can apply.*

#### Evidence 7 — v1.6.0 hooks implement defense-in-depth against symlink attacks (F16)

```bash
grep -nE 'O_NOFOLLOW|lstatSync|isSymbolicLink|renameSync|fchmodSync' /tmp/repo-scan-JuliusBrussee-caveman/hooks/caveman-config.js
```
...
*Classification: Confirmed fact — v1.6.0 code uses every defense expected: atomic temp+rename, O_NOFOLLOW flag, mode 0o600, symlink refusal at parent and target, 64-byte size cap, whitelist validation.*

#### Evidence 8 — No SECURITY.md at repo root (F5 disclosure gap)
...
*Classification: Confirmed fact — the only SECURITY.md is nested inside caveman-compress/ and covers Snyk static-analysis rating, not a disclosure policy. No SECURITY.md at repo root.*

#### Evidence 9 — Dependabot is disabled — even upstream dep vulns won't surface
...
*Classification: Confirmed fact — the response distinguishes 'disabled' from a permissions error. Dependabot is explicitly off.*

#### Evidence 10 — Security PR #70 sat open 5 days before merge (F5 response-lag component, F11 review-gate context)

```bash
gh pr view 70 -R JuliusBrussee/caveman --json createdAt,mergedAt,reviews
```

Result:
```json
{"createdAt":"2026-04-10T..","mergedAt":"2026-04-15T12:50:16Z","reviews":[]}
```

*Classification: Confirmed fact — PR #70 created 2026-04-10, merged 2026-04-15 (5-day lag). Zero reviews in the reviews array. Same pattern for companion PR #71. During those 5 days, 4 vulnerable releases (v1.4.0, v1.4.1, v1.5.0, v1.5.1) shipped.*
...
*Generated by [stop-git-std](https://github.com/stefans71/stop-git-std) deep dive · 2026-04-20 · scanned main @ `c2ed24b3e5d412cd0c25197b2bc9af587621fd99` (v1.6.0) · scanner V2.5-preview-step-g*
