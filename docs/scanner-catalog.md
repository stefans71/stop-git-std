...
The live catalog of all completed scans using the V2.3 scanner pipeline. Each entry records the repo, commit, verdict, shape, and methodology so future scans can:
...
**Current count: 7 entries (6 full scans + 1 re-run determinism record).** JSON-first Trigger #1 fires at 10 full scans.
...
4. **Tag `methodology-used`** on every entry: `path-a` for continuous single-session scans, `path-b` for delegated scans (not yet exercised — see Operator Guide §8.2).
