...
for GitHub Scanner reports and the scan template itself.

Usage:
  python3 validate-scanner-report.py [--report | --template] <path-to-html>
...
Modes:
  (default)    Permissive mode — checks structure, reports placeholder count as
               ...
               report.
  --report     Strict mode for rendered reports. Fails non-zero if any
               {{PLACEHOLDER}} tokens remain or any <!-- EXAMPLE-START/END -->
               markers remain. Also runs the repo-untrusted-text escape check
               (C4) heuristic.
  --template   Verification mode for the canonical template. Requires >0
               ...
               either empty or already a rendered report in disguise).
