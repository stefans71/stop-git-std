...
**Last updated:** 2026-05-06T12:12:35Z

**Scope:** Catalog entries 16–30 (15 V1.2-schema wild scans).
**Date:** 2026-05-06 (entry 30 onecli added in session 16; cross-scan tables §2–§9 still pinned at the n=13 re-derivation done in session 14 morning, with n=14 + n=15 deltas captured in §1 totals + §10 changelog; numbers read directly from the 15 form.json bundles in `docs/scan-bundles/`).
**Status:** Living document — **V13-3 COMPARATOR-CALIBRATION TRIGGER FIRED at n=11** (entry 26 Baileys closed the counting window; CONSOLIDATION closed 2026-04-20 — see §10). Post-trigger cadence (broadened per OD-4): N=25 V1.2 scans OR any of 6 taxonomy-strain events. Currently **15/25** scans + **3/6** taxonomy-strain events (skills + multica + impeccable `missing_qualitative_context` Q2 fires; onecli adds a **first-of-kind Q4 `signal_vocabulary_gap` fire on a design-risk dimension** — MITM-by-design / CA-cert-trust posture — but is recorded as a **new-driver-class observation** rather than a 4th `missing_qualitative_context` taxonomy-strain fire). The §9 V13-1 inference no longer holds at n=13 — see §9 for the contraindication of V13-3 deferred Item C9 and the two-path decision deferred to next trigger.
...
| 29 | pbakaus/impeccable | Multi-harness agent-skills + npm CLI + Chrome ext | Caution | **2** | Q2 **missing_qualitative_context** (3rd Q2 fire; **same close-rate driver as multica** — 2 closed in last week, #92 fix-PR authored = responsive-but-mid-fix) + Q4 **missing_qualitative_context** (channel-count denominator misleading — 23 of 24 detected npm channels are test fixtures under `tests/framework-fixtures/`) |
| 30 | onecli/onecli | Credential-vault for AI agents (TS+Rust monorepo with MITM HTTPS gateway) | Caution | 1 | Q4 **signal_vocabulary_gap** (**first Q4 design-risk fire** — MITM-by-design / CA-cert-trust posture; gateway requires installing a generated CA as trusted root → all agent HTTPS decrypted at proxy. Phase 3 advisory amber FALLBACK is factually correct on installer signals but does not capture architectural-trust-surface implication) |

**Totals (n=15)**: 15 scans, 14 overrides, 3 zero-override (wezterm 21, QuickLook 22, kanata 23). **V13-3 comparator-calibration trigger fired at n=11; CONSOLIDATION closed 2026-04-20.** Post-trigger cadence: 15/25 toward N=25 OR 3/6 taxonomy-strain events. **Entries 27 + 28 + 29 are the three `missing_qualitative_context` taxonomy-strain fires** (skills = sample-floor degeneracy; multica + impeccable = closed-within-window counter-signal — impeccable is the second instance of the multica driver, NOT a new-driver escalation). **Entry 30 (onecli) is a different override class** — Q4 `signal_vocabulary_gap` on a NEW design-risk dimension (MITM-by-design / CA-cert-install-required); recorded as a new-driver-class observation rather than a 4th taxonomy-strain fire. The n=11-bounded inference "the V13-1 split was correct and complete; the catchall hasn't fired" remains broken — see §9 for full revision and the contraindication of V13-3 deferred Item C9.

---

## §2 · Override-enum distribution

**11 overrides across 13 scans** (0.85 overrides/scan; 10 of 13 scans fired ≥1 override = 77% scan-level participation). _Tables in §2–§9 below are pinned at the n=13 re-derivation done in session 14; entries 29 (impeccable, 2 overrides — Q2 + Q4 both `missing_qualitative_context`) and 30 (onecli, 1 override — Q4 `signal_vocabulary_gap`) are captured in §1 totals + §10 changelog entries. New cumulative numbers at n=15: 14 overrides / 15 scans = 0.93 overrides/scan; 12/15 = 80% scan-level participation; `signal_vocabulary_gap` × 7 (regaining modal lead after onecli's Q4 fire); `missing_qualitative_context` × 4._
...
Shape distribution per `phase_3_advisory.shape_classification.category` at n=15 (from `compute.classify_shape()`):
...
| library-package | 2 | Baileys (26), **impeccable (29) — MISCLASSIFIED** (real shape is agent-skills-collection; Step 2 manifest_files==0 gate too strict — see §8 V12x-18) |
| embedded-firmware | 1 | WLED (25) |
| agent-skills-collection | 1 | skills (27) — also a near-misclassification: skills was Markdown-canonical so Step 2's secondary branch (`primary_lang == "markdown" and len(agent_rule_files) >= 3`) caught it; impeccable's primary_language is JavaScript and its 36 source skills + 468 fanout copies were not enumerated by the harness, so neither Step 2 branch fires |
| agentic-platform | 2 | multica (28), **onecli (30) — credential-vault subshape**, first hit on Step 7.5 from a non-monorepo-hint path: cross-language TS+Rust monorepo (`monorepo.inner_packages` includes both npm and Cargo) + publishable npm at root + backend dir non-canonical (Rust gateway lives at `apps/gateway`, npm packages at `apps/web` + `packages/{db,ui,eslint-config}`). First scan pairing agentic-platform with **MITM-by-design / CA-cert-trust** posture — see §8 V12x-19 candidate |

**`agentic-platform` Step 7.5 branch landed 2026-05-02** (merge `c1ae04d`; full-detail commit `f90e508`). The compound signal — `monorepo.is_monorepo == True` AND `monorepo.inner_packages` contains both npm + a non-npm ecosystem AND backend manifest under `server/`/`backend/`/`api/`/`services/server/` AND publishable npm at root or `packages/<n>/`/`apps/<n>/` — fires high-confidence on multica (entry 28, the first V1.2 hit). 12 other bundles are unchanged: kanata + wezterm are single-ecosystem monorepos (crates.io only) and stay cli-binary; the remaining 10 are not monorepos at all. The plan §Current state recommendation cited `docker-compose.selfhost*.yml` as the primary signal; that string only surfaces in phase_1 indirectly via `code_patterns.dangerous_primitives.tls_cors.files[].file` when CORS patterns happen to fire, so the implementation substituted the cleaner `monorepo.inner_packages` cross-language signal that captures the same multi-stack-platform structure deterministically.
...
| ~~V12x-17~~ | ~~agentic-platform classifier branch~~ | **LANDED 2026-05-02** (merge `c1ae04d`; commit `f90e508`). Step 7.5 between cli-binary and library-package fires high-confidence on multica via compound signal: cross-language monorepo (npm + non-npm ecosystems in `monorepo.inner_packages`) + backend manifest under `server/`/`backend/`/`api/`/`services/server/` + publishable npm at root or `packages/<n>/` / `apps/<n>/`. The plan's `docker-compose.selfhost*.yml` recommendation was substituted for the cleaner `monorepo.inner_packages` signal (the compose-file string only surfaces in phase_1 indirectly via tls_cors hits). 12 other V1.2 bundles unchanged. Multica's stored `phase_3_advisory.shape_classification` refreshed to `agentic-platform` (high confidence). | ~~Medium~~ → **DONE** |
| **V12x-18** | **Step 2 agent-skills-collection heuristic too strict — manifest_files==0 rejects repos that ship skills + a CLI helper** | **2/14 scans affected — impeccable (29) AND skills (27, JavaScript primary). Need a signal-ratio check: if skill_files >> manifest_files, classify as agent-skills-collection regardless of npm manifest presence. Specifically: if recursive-enumerated `<harness>/skills/**/*.md` file count exceeds publishable-manifest count by ≥10×, override Step 8's library-package match to agent-skills-collection.** | **Medium** (depends on F5 harness-recursion patch landing first; without recursive enumeration the ratio cannot be computed) |
| **V12x-19** | **Q4 design-risk signal — `q4_design_requires_root_trust` for MITM-by-design / CA-cert-install / kernel-module / system-wide-LD_PRELOAD postures** | **1/15 scans (onecli 30) — first Q4 design-risk fire. Phase 3 advisory FALLBACK is factually correct on installer signals but does not capture the architectural-trust-surface implication of designs that require trusting a generated CA cert, kernel module, root daemon, or system-wide LD_PRELOAD. Couple with shape × maturity modifiers: agentic-platform + < 6mo + solo-maintained + design_requires_root_trust → red. Detection sources: README pattern matching for "install our CA", "trusted root", "MITM", "kernel module", "root daemon", "LD_PRELOAD"; presence of `rcgen`/`mkcert`/CA-related crypto crates in Cargo.toml; presence of system-wide install-script paths (`/etc/`, `/usr/local/share/ca-certificates/`).** | **Low–Medium** (1 fire, but fires on a high-stakes shape — credential vaults / agent secrets brokers) |
...
**Decision deferred** to the next V13-3 follow-up event (N=25 OR any of the remaining 3 taxonomy-strain events; currently 14/25 + 3/6 strain). The third `missing_qualitative_context` fire has now arrived (impeccable Q2, 2026-05-02 session 15) but it carries the **same close-rate counter-signal driver as multica (entry 28)** — NOT a new semantic driver. Per the watch language, this confirms the multica pattern is recurring (n=2 close-rate fires) without escalating to board review. If a *fourth* `missing_qualitative_context` fire arrives with a genuinely third-distinct semantic driver, escalate to board review immediately rather than wait for the N=25 trigger — that would indicate the catchall is genuinely unbounded, not just under-split. (Current driver tally: sample-floor degeneracy × 1 = skills 27; close-rate counter-signal × 2 = multica 28 + impeccable 29.)
...
- **2026-05-02 (V12x-17 LANDED — session 14)** — `classify_shape()` Step 7.5 agentic-platform branch merged (`c1ae04d`; commit `f90e508`). Compound signal: cross-language monorepo (`monorepo.inner_packages` containing both npm + a non-npm ecosystem) + backend manifest under `server/`/`backend/`/`api/`/`services/server/` + publishable npm at root or `packages/<n>/` / `apps/<n>/`. The plan recommendation cited `docker-compose.selfhost*.yml`; that string only surfaces in phase_1 indirectly via tls_cors hits, so the implementation substituted `monorepo.inner_packages` as a cleaner deterministic signal capturing the same multi-stack-platform structure. Multica (entry 28) is the first V1.2 hit (high confidence); 12 other bundles unchanged (kanata + wezterm are single-ecosystem Cargo workspaces — no fire; remaining 10 are not monorepos). §6 shape table updated: library-package 2→1; agentic-platform 0→1. Multica's stored `phase_3_advisory.shape_classification` refreshed from `library-package` to `agentic-platform` (high) — scorecard hints unchanged (FALLBACK rules are shape-agnostic). 31 new tests in `tests/test_classify_shape.py`; full suite 652/652. Owner approved direct merge (no board review) — narrow single-shape addition, zero regressions.
- **2026-05-02 (n=13 re-derivation — session 14)** — Cross-scan tables §2–§9 re-derived from the 13 form.json bundles in `docs/scan-bundles/` (back-to-basics-plan §Current state follow-up #6 closed). Headline shifts: signal_vocabulary_gap modal share dropped 67%→55% as `missing_qualitative_context` fires twice consecutively (skills + multica) with distinct drivers (sample-floor degeneracy + closed-within-window counter-signal); Q4 override hotspot share dropped 67%→45% with Q2 climbing 22%→36%; override streak now 5 (entries 24–28); zero-override scans unchanged at 3 (21/22/23). Five new V12x backlog items derived: V12x-15 (Q2 sample-floor signal), V12x-16 (Q2 close-rate counter-signal), V12x-17 (agentic-platform classifier branch). §5.2 silent-fix corrected from 11/11 to 12/13 (ghostty was mis-recorded as 0 advisories at n=11; current bundle shows 5 GHSA published). §6 shape-classifier table added (desktop-app 4, cli-binary 3, specialized-domain-tool 2, library-package 2 [incl. multica misclassified — see V12x-17 LANDED entry above], embedded-firmware 1, agent-skills-collection 1). §9 V13-1 follow-up rewritten: V13-1 split holds, but `missing_qualitative_context` carries genuine semantic load → V13-3 deferred Item C9 (prune the catchall) is now contraindicated; two paths (keep-as-is OR split into `sample_floor_degeneracy` + `responsive_close_rate_counter_signal`) deferred to next V13-3 trigger event.
...
- **2026-05-02 (entry 29 update — session 15, pbakaus/impeccable wild scan)** — Added pbakaus/impeccable (29) to roster. **14th V1.2 wild scan; second multi-harness skills entry** (after mattpocock/skills entry 27); first scan that pairs a skills collection with a published npm CLI. 24.3k-star JavaScript design-anti-pattern toolkit; Apache-2.0; 5.5 months old. **Two Q-cell overrides** (first time a single scan fires two `missing_qualitative_context` overrides): Q2 (close-rate counter-signal — 2 security issues closed in last week + #92 fix-PR authored against 22d-old open item = responsive-but-mid-fix posture); Q4 (channel-count denominator misleading — harness emitted 24 npm channels but 23 are test fixtures under `tests/framework-fixtures/`, only `impeccable@2.1.8` is real). **Q2 fire is the 3rd `missing_qualitative_context` Q2 fire (skills + multica + impeccable)** but carries the **same close-rate driver as multica** — NOT a new-driver escalation per the §9 watch language. Driver tally now: sample-floor degeneracy × 1 (skills 27); close-rate counter-signal × 2 (multica 28 + impeccable 29). **Shape misclassification noted (NEW)**: classify_shape() returned `library-package` (medium confidence) — but the dominant shape signal is multi-harness agent-skills collection (468 skill files across 13 harness directories vs single npm CLI). Step 2 agent-skills-collection branch was rejected by `manifest_files == 0` AND `executable_files == 0` AND `releases_count == 0` AND `primary_lang in {shell, markdown, ""}` — too strict for repos that ship a skills collection alongside a CLI helper. Same too-strict gate also rejects entry 27 (mattpocock/skills, JavaScript primary; only caught by the secondary Markdown-canonical branch because skills is markdown-primary). New V12x backlog item (V12x-18) — Step 2 needs a signal-ratio check: if recursive-enumerated `<harness>/skills/**/*.md` file count exceeds publishable-manifest count by ≥10×, override Step 8's library-package match to agent-skills-collection. Depends on F5 harness-recursion patch landing first. V13-3 follow-up at 14/25 toward N=25 cadence; G4 taxonomy-strain count: 3 of 6 (impeccable Q2 same-driver counts as a strain event but not as a new-driver escalation). 14/14 V1.2 wild scans with 0 published GHSA advisories.
- **2026-05-06 (entry 30 update — session 16, onecli/onecli wild scan)** — Added onecli/onecli (30) to roster. **15th V1.2 wild scan; first credential-vault / agent-secrets-broker shape entry** in the catalog. 2,093-star TS+Rust monorepo by `onecli` org; Apache-2.0; ~2 months old (created 2026-03-08; org 2026-02-24). Architecture: Rust HTTP gateway (`apps/gateway`) does **MITM HTTPS interception** via a `rcgen 0.13`-generated CA certificate that agents must trust as a root authority. Encryption is AES-256-GCM via `ring 0.17` (audited); OSS edition auto-generates a 32-byte key on first start to `/app/data/secret-encryption-key` (mode 600); Cloud edition uses AWS KMS via Cargo `cloud` feature. Bitwarden integration via `ap-{client,proxy-client,proxy-protocol,noise}` v0.9.0 crates (Bitwarden Agent Access protocol — separate Noise-based authenticated channel). Solo-maintained (guyb1 85.0%; johnnyfish 12.0%; 15 contributors). **Active branch ruleset on main** (id 13642592, enforcement=active) — meaningfully different from impeccable entry 29's no-protection posture — but ruleset does not enforce required-reviewer count: 0.0% formal review rate over 208 sampled merged PRs (entire merged-lifetime population). 6 findings (4 Warning + 2 Info), Caution single verdict. **One Q-cell override**: Q4 `signal_vocabulary_gap` — **first wild V1.2 scan to fire `signal_vocabulary_gap` for a Q4 design-risk dimension**. Phase 3 advisory amber FALLBACK is factually correct on installer signals (no pinned channels, no artifact verification) but does not capture the architectural-trust-surface implication of MITM-by-design / CA-cert-install posture. Once the agent trusts the CA, every HTTPS call the agent makes flows through the gateway in cleartext — not just calls to APIs configured for credential injection. Suggests adding a `q4_design_requires_root_trust` signal (CA-cert install required, kernel module, root daemon, system-wide LD_PRELOAD) coupled with shape × maturity modifiers (agentic-platform + < 6mo + solo-maintained + design_requires_root_trust → red). Shape correctly classified `agentic-platform` (medium confidence) on first attempt — cross-language TS+Rust monorepo + publishable npm + backend dir non-canonical. Other findings: F0 solo-maintainer + no CODEOWNERS on a 2-month-old credential vault; F1 active ruleset but no required-reviewer rule + 0.0% formal review on 208 PRs; F2 `curl|sh` installer trusts three mutable anchors (onecli.sh DNS, main branch compose URL, latest Docker tag); F4 no SECURITY.md / no GHSA / no dependabot.yml despite responsive past-fix history (#131 0.0.0.0 binding fixed in days); F5 scanner coverage gaps (gitleaks unavailable, OSSF 404 - repo too new, Dependabot 403 admin scope). New V12x backlog candidate: V12x-19 — add `q4_design_requires_root_trust` signal to compute.derive_signals(); rule-table addition for agentic-platform + new-repo + solo + design-requires-trust → red. **Taxonomy-strain accounting**: onecli's Q4 `signal_vocabulary_gap` is recorded as a **new-driver-class observation** (first design-risk Q4 fire) rather than a 4th `missing_qualitative_context` taxonomy-strain fire — the §9 watch language is specifically about `missing_qualitative_context` accumulation. V13-3 follow-up at 15/25 toward N=25 cadence; G4 taxonomy-strain count remains 3 of 6 (skills sample-floor + multica close-rate + impeccable close-rate). 15/15 V1.2 wild scans with 0 published GHSA advisories.
