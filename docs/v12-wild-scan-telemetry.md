...
**Totals (n=12)**: 12 scans, 10 overrides, 3 zero-override. **V13-3 comparator-calibration trigger originally fired at n=11**; re-trigger threshold N=25 OR any of 6 taxonomy-strain events. **Entry 27 is a taxonomy-strain candidate**: first `missing_qualitative_context` fire across all 12 V1.2 wild scans (was × 0 at n=11) — the V13-3 §2 inference that "the V13-1 split was correct and complete" was n=11-bounded; n=12 reopens whether `missing_qualitative_context` is a stable enum value or whether `sample_floor_degeneracy` (or similar) deserves its own enum.
