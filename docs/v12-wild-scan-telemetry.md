...
**Scope:** Catalog entries 16–29 (14 V1.2-schema wild scans).
**Date:** 2026-05-02 (cross-scan tables §2–§9 re-derived for n=13 in session 14 morning; entry 29 impeccable added in session 15; numbers read directly from the 14 form.json bundles in `docs/scan-bundles/`).
**Status:** Living document — **V13-3 COMPARATOR-CALIBRATION TRIGGER FIRED at n=11** (entry 26 Baileys closed the counting window; CONSOLIDATION closed 2026-04-20 — see §10). Post-trigger cadence (broadened per OD-4): N=25 V1.2 scans OR any of 6 taxonomy-strain events. Currently **14/25** scans + **3/6** taxonomy-strain events (skills + multica + impeccable `missing_qualitative_context` Q2 fires; impeccable is the 3rd Q2 fire but **same close-rate driver as multica**, NOT a new-driver escalation per the §9 watch language). The §9 V13-1 inference no longer holds at n=13 — see §9 for the contraindication of V13-3 deferred Item C9 and the two-path decision deferred to next trigger.
...
**Totals (n=14)**: 14 scans, 13 overrides, 3 zero-override (wezterm 21, QuickLook 22, kanata 23). **V13-3 comparator-calibration trigger fired at n=11; CONSOLIDATION closed 2026-04-20.** Post-trigger cadence: 14/25 toward N=25 OR 3/6 taxonomy-strain events. **Entries 27 + 28 + 29 are the three taxonomy-strain fires**: three consecutive `missing_qualitative_context` Q2 overrides with two distinct semantic drivers (skills = sample-floor degeneracy; multica + impeccable = closed-within-window counter-signal). The third fire (impeccable) is the **second instance of the multica close-rate driver** — confirms recurring pattern but is NOT a new-driver escalation under the §9 watch language. The n=11-bounded inference "the V13-1 split was correct and complete; the catchall hasn't fired" remains broken — see §9 for full revision and the contraindication of V13-3 deferred Item C9.

---

## §2 · Override-enum distribution

**11 overrides across 13 scans** (0.85 overrides/scan; 10 of 13 scans fired ≥1 override = 77% scan-level participation). _Tables in §2–§9 below are pinned at the n=13 re-derivation done in session 14; entry 29 (impeccable, 2 overrides — Q2 + Q4 both `missing_qualitative_context`) is captured in §1 totals + the §10 changelog entry. New cumulative numbers at n=14: 13 overrides / 14 scans = 0.93 overrides/scan; 11/14 = 79% scan-level participation; `missing_qualitative_context` × 4 (now modal-tied with `signal_vocabulary_gap` × 6 if you count by enum-value distinct uses across cells)._
...
Shape distribution per `phase_3_advisory.shape_classification.category` at n=14 (from `compute.classify_shape()`):
